...
internal class TitleBarHelper
{
    ...
    public static void UpdateTitleBar(ElementTheme theme)
    {
        if (App.MainWindow.ExtendsContentIntoTitleBar)
        {
            ...
            if (hwnd == Win32Helper.GetActiveWindow())
            {
                Win32Helper.SendMessage(hwnd, WMACTIVATE, WAINACTIVE, IntPtr.Zero);
                Win32Helper.SendMessage(hwnd, WMACTIVATE, WAACTIVE, IntPtr.Zero);
            }
            else
            {
                Win32Helper.SendMessage(hwnd, WMACTIVATE, WAACTIVE, IntPtr.Zero);
